...
extract_jwt_token() {
    local response="$1"
    echo "$response" | grep -o '"token"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d':' -f2 | tr -d ' "'
}
...
validate_response_value() {
    ...
    case "$comparison_type" in
        "equals")
            # Normalize numbers to ensure format consistency (e.g., 50 == 50.0)
            normalized_actual=$(printf "%.1f" "$actual_value" 2>/dev/null || echo "$actual_value")
            normalized_expected=$(printf "%.1f" "$expected_value" 2>/dev/null || echo "$expected_value")
...
api_test() {
    ...
    if validate_status_range "$actual_status" "$expected_status_range"; then
        # If this is a login endpoint, extract JWT token
        if [[ "$endpoint" == *"/auth/login" ]]; then
            JWT_TOKEN=$(extract_jwt_token "$response_body")
            echo -e "  ${BLUE}JWT token extracted: $JWT_TOKEN${NC}"
        fi
...
validate_response_value "$LAST_RESPONSE" "isCustom" "true"
